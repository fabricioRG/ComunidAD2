...
export class RequestComunityComponent implements OnInit {
  ...
  aprobarSolicitud(elemento: any) {
    ...
    modal.componentInstance.modalBodyTitle =
      '¿Estás seguro que deseas Aprobar la solicitud del usuario: ' +
      elemento.user.registroAcademico +
      '?';
    modal.componentInstance.modalBody =
      'Si aceptas se cambiara el estado del usuario a: ' +
    ...
    modal.result.then(
      (result) => {
        ...
        this.filtrarAsignacionesComunidad
          ...
          .subscribe(
            ...
            (error: any) => {
              this.mostrarMensaje(
                'Error al actualizar aceptacion de la solicitud',
                'No se pudo completar la solicitud de cambio',
                error
              );
            }
          );
      ...
      (reason) => {
        this.mostrarMensaje(
          'No se acepto la Aceptacion de la solicitud',
          'No se pudo completar la solicitud de cambio',
          'No aceptado'
        );
      }
  ...
  denegarSolicitud(elemento: any) {
    ...
    modal.componentInstance.modalBodyTitle =
      '¿Estás seguro que deseas denegar la solicitud del usuario: ' +
      elemento.user.registroAcademico +
      '?';
    modal.componentInstance.modalBody =
      'Si aceptas se cambiara el estado del usuario a: ' +
    ...
    modal.result.then(
      (result) => {
        ...
        this.filtrarAsignacionesComunidad
          ...
          .subscribe(
            ...
            (error: any) => {
              this.mostrarMensaje(
                'Error al actualizar denegacion de la solicitud',
                'No se pudo completar la solicitud de cambio',
                error
              );
            }
      ...
      (reason) => {
        this.mostrarMensaje(
          'No se acepto la denegacion de la solicitud',
          'No se pudo completar la solicitud de cambio',
          'No aceptado'
        );
      }
