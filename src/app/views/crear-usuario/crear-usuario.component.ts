...
export class CrearUsuarioComponent implements OnInit {
  ...
  ngOnInit() {
    this.signupForm = this._builder.group({
      numeroCarnet: ['',[Validators.required,Validators.max(999999999),Validators.min(100000000)]],
      ...
      contrasena: ['',[Validators.required,Validators.minLength(6),Validators.maxLength(30),Validators.pattern(this.REGEX_PASSWORD)]],
      repetirContrasena: ['',[Validators.required,Validators.minLength(6),Validators.maxLength(30),Validators.pattern(this.REGEX_PASSWORD)]],
  ...
  enviar(values: any){
    ...
    if(values.contrasena != values.repetirContrasena){
      contadorErrores++
      mensajesError = this.agregarMensajeError(contadorErrores,mensajesError,'Las contrasenas no coinciden\n')
      send = false;
    }
    console.log('Fecha nacimiento: '+values.fechaNacimiento)
    console.log('Fecha actual: '+this.obtenerFechaActual())
    if(!this.compararFechas(values.fechaNacimiento, this.obtenerFechaActual())){
      contadorErrores++
      mensajesError = this.agregarMensajeError(contadorErrores,mensajesError,'La fecha de nacimiento no puede ser mayor o igual a la fecha actual\n')
      send = false;
    }

    if(send){
      console.log("Esta fecha nacimiento es: "+this.convertirFecha(values.fechaNacimiento))
      ...
      this.dataService.addNewUser(usuario).subscribe(user => {
  ...
  compararFechas(fechaNacimiento: string, fechaActual: string){
    ...
    if( (new Date(fechaNacimiento).getTime() >= new Date(fechaActual).getTime())){
      return false;
    }else{
      return true;
    }
